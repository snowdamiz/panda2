...
- The backend fetches the latest blockhash, serializes the native SOL transfer plus memo/reference, binds one submitted signature to the order, and submits signed transaction bytes through Solana RPC. Confirmation polling uses `getSignatureStatuses`; the heavier `getTransaction` call runs only once the configured commitment is reached. Verification rejects token transfers, requires one matching native SOL transfer to the treasury wallet, requires the order memo/reference, and only accepts transactions at or above the configured confirmation threshold.
- Keep Alchemy usage alerts and a monthly usage limit enabled as a provider-side backstop. The application also rejects malformed signatures and oversized transactions before RPC, deduplicates repeated submissions, coalesces concurrent verification, rate-limits payment writes, and never exposes the RPC URL to the browser.
