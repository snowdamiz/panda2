...
func youtubeClipSchema() json.RawMessage {
	return schemaWithProperties([]string{"query"}, map[string]any{
		...
		"caption_instructions": map[string]any{
			"type":        "string",
			"description": "Optional subtitle style or placement guidance from the user, such as big captions, captions at the top, white Inter font, green medium outline, thick border, caption bubble/background color and opacity, mix it up, make the captions colorful, choose caption colors from the clip/video palette, do not cover the facecam, avoid source captions, or use per-speaker captions. Omit unless the user provided caption-specific guidance.",
		},
		"language": map[string]any{
			"type":        "string",
			"description": "Optional spoken language hint, such as english, spanish, french, or japanese. Omit unless the user specifies the video's language.",
