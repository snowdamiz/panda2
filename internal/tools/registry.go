...
func DefaultDefinitions() []Definition {
	definitions := []Definition{
		...
		adminWrite("discord.create_role", "Dry-run or confirmed creation of a Discord role with no elevated permissions by default. Requires Panda's bot role to have Manage Roles and sufficient role hierarchy.", []string{"name"}, "MANAGE_ROLES"),
		...
		},
		{
			...
			Description:           "Read or update Panda's agent soul: response style, personality, and voice for the current guild. Use status for brainstorming/context; use set/update only after the user clearly asks to save, apply, set, or update a specific soul.",
		...
		{
			Name:                  "panda.manage_member_role",
			Description:           "Prepare adding or removing a Discord role from a guild member. Use this after a role profile is configured when an owner asks to make a user admin/mod by assigning that Discord role. Provide user_id plus role_id, a role mention, or role_name.",
			RequiredPermission:    admin.PermissionAdminConfigWrite,
			ToolClass:             ToolClassAdminWrite,
			InputSchema:           actionSchema([]string{"action", "user_id"}, "action", "user_id", "role_id", "role_name", "dry_run"),
			OutputSchema:          objectSchema("result"),
			Timeout:               2 * time.Second,
			Redaction:             RedactSecrets,
			Audit:                 AuditOnUse,
		...
		{
			Name:                  "panda.manage_discord_role",
			Description:           "Prepare creating a new Discord role by name. Use this when an admin asks Panda to create, make, or add a server role. Created roles start with no elevated permissions; Discord may reject execution until Panda's bot role has Manage Roles and sufficient role hierarchy.",
			RequiredPermission:    admin.PermissionAdminConfigWrite,
			ToolClass:             ToolClassAdminWrite,
			InputSchema:           actionSchema([]string{"action", "name"}, "action", "name", "dry_run"),
			OutputSchema:          objectSchema("result"),
			Timeout:               2 * time.Second,
			Redaction:             RedactSecrets,
			Audit:                 AuditOnUse,
		...
		},
		{
			...
			Description:           "Draft, preview, inspect, approve, pause, resume, disable, archive, run, simulate, export, or roll back user-created composed tools and event-triggered automations through natural-language admin requests.",
		...
		},
		{
			...
			Description:           "Call this before answering questions about what tools or capabilities Panda has. It lists callable tools in the current guild and channel context, including built-in native tools and enabled composed tools. For users without admin permissions, admin-only tool names and details are withheld and summarized generically.",
			RequiredPermission:    admin.PermissionAssistantUse,
