...
// YoutubeClip records a clip generated by panda.clip_youtube so its creator can
// revisit it in the clips portal. Named YoutubeClip (not YouTubeClip) so GORM's
// default pluralization resolves to "youtube_clips"; TableName() pins it anyway.
type YoutubeClip struct {
	...
	ClipType           string    `gorm:"not null;default:''"`
	ObjectKey          string    `gorm:"not null"`
	ThumbnailObjectKey string    `gorm:"not null;default:''"`
	DurationSeconds    float64   `gorm:"not null;default:0"`
	SourceStartSeconds float64   `gorm:"not null;default:0"`
	SourceEndSeconds   float64   `gorm:"not null;default:0"`
	VideoTitle         string    `gorm:"not null;default:''"`
	VideoURL           string    `gorm:"not null;default:''"`
	VideoUploader      string    `gorm:"not null;default:''"`
	SizeBytes          int64     `gorm:"not null;default:0"`
	ViralityScore      int       `gorm:"not null;default:0"`
	HookScore          int       `gorm:"not null;default:0"`
	RetentionScore     int       `gorm:"not null;default:0"`
	ShareabilityScore  int       `gorm:"not null;default:0"`
	CreatedAt          time.Time `gorm:"index;not null"`
}
